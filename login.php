...
if($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $sql = "SELECT * FROM registration WHERE email = '$email' AND typepassword = '$typepassword'";
  $result = $conn->query($sql);

  // Check if any row matches the credentials
  if($result->num_rows > 0) {
    $row = $result->fetch_assoc(); // Retrieve the user details
    $firstname = $row['firstname']; // Assuming 'firstname' is the column name for the first name in your database

    // Start the session to store user details (including the first name)
    session_start();
    $_SESSION['firstname'] = $firstname; // Store the first name in the session

    // Redirect to index_loggedin.php
    ...
    <div class="card-body">
      <form method="post" action="#" autocomplete="off" onsubmit="return validateForm()">
        ...
        <div class="form-group">
          ...
          <input required="" class="form-control" name="typepassword" id="typepassword" type="password">
        </div>
        <p class="signup-link">Don't have an account yet? <a href="register.php" class="signup-btn">Sign Up!</a></p>
        <p class="signup-link"><a href="forgot_password.php" class = "signup-btn">Forgot Password?</a></p>
