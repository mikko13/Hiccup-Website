...
if(isset($_POST["submit"])) {
  ...
  try {
    $sql = "INSERT INTO registration (firstname, lastname, email, typepassword)
        VALUES (?, ?, ?, ?)"; // Using prepared statement placeholders

    $stmt = $conn->prepare($sql);
    $stmt->bind_param('ssss', $firstname, $lastname, $email, $password); // 'ssss' indicates string parameters

    $stmt->execute();

    header("Location: login.php"); // Redirect after insertion
...
<header>
  <img src="logo.png" alt="Hiccup Logo" class="hiccup-logo" />
  <a href="index.html"></a>
...
<body>
  <div class="card">
    ...
    <div class="card-body">
      <form autocomplete="off" id="registrationForm" method="post" action="register.php">
        <div class="form-group">
          ...
          <input type="password" id="typepassword" name="typepassword" required><br><br>

          <label for="confirmpassword">Confirm Password:</label>
          <input type="password" id="confirmpassword" name="confirmpassword" required><br><br>

          <p class="signup-link">Already have an account? <a href="login.php" class="signup-btn">Login!</a></p>
    ...
    <script>
      document.getElementById('registrationForm').addEventListener('submit', function (event) {
        ...
        if (password !== confirmPassword) {
          alert('Passwords do not match! Please check and try again.'); // Display an alert
...
<footer>
  <img src="logo_circle.png" alt="circlelogo" class="circlelogo">
  <div class="text-footer"> Hiccup - Baliwag City, Bulacan <br>
    <span class="address">764 Benigno S. Aquino Avenue, Poblacion, Baliuag, Philippines</span>
  </div>
  <div class="text-footer2"> Visit Us <br>
